...
func (oq *OrderQuery) QuerySender() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, order.SenderTable, order.SenderColumn),
		)
...
func (oq *OrderQuery) QueryRecipient() *UserQuery {
	query := (&UserClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(order.Table, order.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, order.RecipientTable, order.RecipientColumn),
		)
		fromU = sqlgraph.SetNeighbors(oq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (oq *OrderQuery) QueryNotifications() *NotificationQuery {
	query := (&NotificationClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, order.NotificationsTable, order.NotificationsColumn),
		)
		fromU = sqlgraph.SetNeighbors(oq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withSender:        oq.withSender.Clone(),
		withRecipient:     oq.withRecipient.Clone(),
		withNotifications: oq.withNotifications.Clone(),
...
func (oq *OrderQuery) WithSender(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithRecipient(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withRecipient = query
	return oq
}

// WithNotifications tells the query-builder to eager-load the nodes that are connected to
// the "notifications" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithNotifications(opts ...func(*NotificationQuery)) *OrderQuery {
	query := (&NotificationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withNotifications = query
	return oq
}
...
//
//	client.Order.Query().
//		GroupBy(order.FieldTrackingNumber).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Order.Query().
//		Select(order.FieldTrackingNumber).
//		Scan(ctx, &v)
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		loadedTypes = [3]bool{
			oq.withSender != nil,
			oq.withRecipient != nil,
			oq.withNotifications != nil,
		}
	)
	if oq.withSender != nil || oq.withRecipient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withSender; query != nil {
		if err := oq.loadSender(ctx, query, nodes, nil,
			func(n *Order, e *User) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withRecipient; query != nil {
		if err := oq.loadRecipient(ctx, query, nodes, nil,
			func(n *Order, e *User) { n.Edges.Recipient = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withNotifications; query != nil {
		if err := oq.loadNotifications(ctx, query, nodes,
			func(n *Order) { n.Edges.Notifications = []*Notification{} },
			func(n *Order, e *Notification) { n.Edges.Notifications = append(n.Edges.Notifications, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		if nodes[i].user_sent_orders == nil {
			continue
		}
		fk := *nodes[i].user_sent_orders
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_sent_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadRecipient(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		if nodes[i].user_received_orders == nil {
			continue
		}
		fk := *nodes[i].user_received_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_received_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadNotifications(ctx context.Context, query *NotificationQuery, nodes []*Order, init func(*Order), assign func(*Order, *Notification)) error {
	...
	query.Where(predicate.Notification(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.NotificationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_notifications
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_notifications" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_notifications" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
