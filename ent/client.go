...
// QueryOrder queries the order edge of a Notification.
func (c *NotificationClient) QueryOrder(n *Notification) *OrderQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, notification.OrderTable, notification.OrderColumn),
		)
		fromV = sqlgraph.Neighbors(n.driver.Dialect(), step)
		return fromV, nil
	}
...
// QuerySender queries the sender edge of a Order.
func (c *OrderClient) QuerySender(o *Order) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := o.ID
		step := sqlgraph.NewStep(
...
// QueryRecipient queries the recipient edge of a Order.
func (c *OrderClient) QueryRecipient(o *Order) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := o.ID
		step := sqlgraph.NewStep(
...
// QueryNotifications queries the notifications edge of a Order.
func (c *OrderClient) QueryNotifications(o *Order) *NotificationQuery {
...
// QuerySentOrders queries the sent_orders edge of a User.
func (c *UserClient) QuerySentOrders(u *User) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
// QueryReceivedOrders queries the received_orders edge of a User.
func (c *UserClient) QueryReceivedOrders(u *User) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Notification, Order, User []ent.Hook
	}
	inters struct {
		Notification, Order, User []ent.Interceptor
	}
)
